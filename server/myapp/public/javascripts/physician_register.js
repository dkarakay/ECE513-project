...
$(document).ready(function () {
    ...
    function physicianSignup(event) {
        ...
        .done(function (data, textStatus, jqXHR) {
            $("#physicianRxData").html(`<pre>${JSON.stringify(data, null, 2)}</pre>`);
            if (data.success) {
                // Redirect to physician login after 1 second
                setTimeout(function () {
                    window.location = "physician_login.html"; // Ensure this page exists
                }, 1000);
            }
        })
        .fail(function (jqXHR, textStatus, errorThrown) {
            if (jqXHR.status == 404) {
                ...
                $("#physicianRxData").html(`<pre>${JSON.stringify(jqXHR.responseJSON, null, 2)}</pre>`);
            }
        });
    }
